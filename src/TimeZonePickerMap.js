...
export const renderTextWithHighlights = (text, matchIndices) => {
  ...
  for (let matchPairIndex in matchIndices) {
    const currentPair = matchIndices[matchPairIndex];
    const nextPair = matchIndices[matchPairIndex + 1]; // TODO will break at end?
    const start = currentPair[0];
    const end = currentPair[1];

    matching.push(text.slice(start, end));
    nonMatching.push(text.slice(end + 1, nextPair[0]));
  }

  matching = matching.map((substring) => (
    <span className="match">{substring}</span>
  ));
