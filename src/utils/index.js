...
export const splitIntoBoundedGroups = (text, matchIndices) => {
  let nonMatching = [];
  let matching = [];
  ...
  const arr = alternate(
    nonMatching.map((nm) => ({ matches: false, text: nm })),
    matching.map((m) => ({ matches: true, text: m }))
  );
  return arr;
